...
func TestChainExportImportFull(t *testing.T) {
	...
	sm, err := stmgr.NewStateManager(cs, consensus.NewTipSetExecutor(filcns.RewardFunc), nil, filcns.DefaultUpgradeSchedule(), cg.BeaconSchedule(), datastore.NewMapDatastore())
	if err != nil {
		t.Fatal(err)
	}
